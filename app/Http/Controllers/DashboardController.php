...
class DashboardController extends Controller
{
    ...
    /**
     * Tableau par company sur tout le mois (mêmes chiffres que dailyBreakdown
     * mais agrégés par company, avec comparaison au mois précédent à la même
     * période).
     */
    public function companyBreakdown(Request $request): JsonResponse
    {
        // Parse month filter (format: YYYY-MM) — timezone Europe/Paris
        if ($request->filled('month')) {
            [$year, $month] = explode('-', $request->month);
            $start = \Carbon\Carbon::createFromDate($year, $month, 1, 'Europe/Paris')->startOfMonth();
            $end = \Carbon\Carbon::createFromDate($year, $month, 1, 'Europe/Paris')->endOfMonth();
        ...
        // Si le mois sélectionné est le mois en cours, on s'arrête à maintenant
        $currentEnd = ($today->between($start, $end)) ? $now : $end;

        // Mois précédent, même fenêtre (jusqu'au même jour si on est dans le mois en cours)
        $previousMonthStart = $start->copy()->subMonth();
        $dayOfMonth = $today->day;
        $daysInPreviousMonth = $previousMonthStart->copy()->endOfMonth()->day;
        $previousMonthEnd = $previousMonthStart->copy()->addDays(min($dayOfMonth, $daysInPreviousMonth) - 1)->endOfDay();

        // Filtres adaptés à la version "jointe" (colonnes qualifiées)
        $applyFilters = function ($query) use ($request) {
            if ($request->filled('brand_name')) {
                $brands = $this->parseMultiSelect($request->brand_name);
                $query->whereIn('calls.brand_name', $brands);
            }

            if ($request->filled('agent_name')) {
                $agents = $this->parseMultiSelect($request->agent_name);
                $query->whereIn('calls.agent_name', $agents);
            }

            if ($request->filled('callcenter_id')) {
                $callcenters = array_map('intval', $this->parseMultiSelect($request->callcenter_id));
                $query->whereIn('calls.callcenter_id', $callcenters);
            }

            if ($request->filled('carrier')) {
                $carriers = $this->parseMultiSelect($request->carrier);
                $query->whereIn('calls.carrier', $carriers);
            }

            if ($request->filled('provider_id')) {
                $providers = array_map('intval', $this->parseMultiSelect($request->provider_id));
                $query->whereIn('phonenumbers.provider_id', $providers);
            }

            if ($request->filled('company_id')) {
                $companies = array_map('intval', $this->parseMultiSelect($request->company_id));
                $query->whereIn('phonenumbers.company_id', $companies);
            }

            if ($request->filled('source_id')) {
                $sources = array_map('intval', $this->parseMultiSelect($request->source_id));
                $query->where(function ($q) use ($sources) {
                    $q->whereIn('calls.source_id', $sources)
                      ->orWhere(function ($sq) use ($sources) {
                          $sq->whereNull('calls.source_id')
                             ->whereIn('phonenumbers.source_id', $sources);
                      });
                });
            }

            if ($request->filled('min_duration') && $request->min_duration == 'true') {
                $query->where('calls.total_duration', '>=', 10);
            }

            return $query;
        };

        // Agrégation par company sur une période donnée
        $aggregate = function ($from, $to) use ($applyFilters) {
            $query = Call::query()
                ->from('calls')
                ->leftJoin('phonenumbers', 'phonenumbers.phonenumber', '=', 'calls.called')
                ->leftJoin('companies', 'companies.id', '=', 'phonenumbers.company_id');

            $applyFilters($query);

            return $query
                ->whereBetween('calls.called_at', [$from, $to])
                ->whereNotNull('phonenumbers.company_id')
                ->selectRaw('
                    phonenumbers.company_id as company_id,
                    companies.name as company_name,
                    companies.color as company_color,
                    COUNT(*) as calls,
                    COALESCE(SUM(calls.payout), 0) as ca,
                    COALESCE(SUM(calls.payout_source), 0) as reverse,
                    COALESCE(SUM(COALESCE(calls.payout, 0) - COALESCE(calls.payout_source, 0)), 0) as benefice,
                    COALESCE(SUM(calls.total_duration), 0) as total_duration,
                    COALESCE(AVG(calls.total_duration), 0) as avg_duration
                ')
                ->groupBy('phonenumbers.company_id', 'companies.name', 'companies.color')
                ->get()
                ->keyBy('company_id');
        };

        $currentByCompany = $aggregate($start, $currentEnd);
        $previousByCompany = $aggregate($previousMonthStart, $previousMonthEnd);

        // Union des company_id présents dans l'une ou l'autre période
        $companyIds = $currentByCompany->keys()->merge($previousByCompany->keys())->unique();

        $items = $companyIds->map(function ($companyId) use ($currentByCompany, $previousByCompany) {
            $c = $currentByCompany->get($companyId);
            $p = $previousByCompany->get($companyId);

            $calls = $c ? (int) $c->calls : 0;
            $ca = $c ? round((float) $c->ca, 2) : 0;
            $reverse = $c ? round((float) $c->reverse, 2) : 0;
            $benefice = $c ? round((float) $c->benefice, 2) : 0;
            $totalDuration = $c ? (int) $c->total_duration : 0;
            $avgDuration = $c ? round((float) $c->avg_duration) : 0;

            $prevCalls = $p ? (int) $p->calls : null;
            $prevCa = $p ? round((float) $p->ca, 2) : null;
            $prevReverse = $p ? round((float) $p->reverse, 2) : null;
            $prevBenefice = $p ? round((float) $p->benefice, 2) : null;
            $prevTotalDuration = $p ? (int) $p->total_duration : null;
            $prevAvgDuration = $p ? round((float) $p->avg_duration) : null;
            ...
            return [
                ...
                'company_name' => $row->company_name ?? 'Inconnu',
                'company_color' => $row->company_color ?? null,
                'calls' => $calls,
                'ca' => $ca,
                'reverse' => $reverse,
                'benefice' => $benefice,
        ...
        // Totaux (sommes des companies)
        $totalCalls = (int) $items->sum('calls');
        $totalCa = round((float) $items->sum('ca'), 2);
        $totalReverse = round((float) $items->sum('reverse'), 2);
        $totalBenefice = round((float) $items->sum('benefice'), 2);
        $totalDuration = (int) $items->sum('total_duration');
        $avgDuration = $totalCalls > 0 ? (int) round($totalDuration / $totalCalls) : 0;

        $prevTotalCalls = (int) $previousByCompany->sum('calls');
        $prevTotalCa = round((float) $previousByCompany->sum('ca'), 2);
        $prevTotalReverse = round((float) $previousByCompany->sum('reverse'), 2);
        $prevTotalBenefice = round((float) $previousByCompany->sum('benefice'), 2);
        $prevTotalDuration = (int) $previousByCompany->sum('total_duration');
        $prevAvgDuration = $prevTotalCalls > 0 ? (int) round($prevTotalDuration / $prevTotalCalls) : 0;
